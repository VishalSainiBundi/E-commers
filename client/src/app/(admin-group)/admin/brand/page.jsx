  ...
  return (
    <div className="min-h-screen p-10 bg-gray-50 text-gray-900 max-w-full mx-auto">
      ...
      <div className="flex items-center justify-between mb-12 ">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight">Brand Listing</h1>
          <p className="text-gray-500 mt-2">Manage all your product brands.</p>
        </div>
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr className="bg-gray-100 border-b text-left">
              ...
              <th className="p-4 text-sm font-semibold">Top Brand</th>
              <th className="p-4 text-sm font-semibold">On Home</th>
              <th className="p-4 text-sm font-semibold">Best Brand</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {categories.map((cat, index) => (
              <tr key={index} className="hover:bg-gray-50 transition ">
                ...
                {/* STATUS */}
                <td className="p-4">
                  <StatusBtn status={cat.status.toString()} url={`brand/status/${cat._id}`} flag="1" label="status" />
                </td>

                {/* TOP CATEGORY */}
                <td className="p-4">
                  <StatusBtn status={cat.is_top} url={`brand/status/${cat._id}`} flag="3" label="is_top" />
                </td>

                {/* ON HOME */}
                <td className="p-4">
                  <StatusBtn status={cat.on_home} url={`brand/status/${cat._id}`} flag="2" label="on_home" />
                </td>

                {/* BEST CATEGORY */}
                <td className="p-4">
                  <StatusBtn status={cat.is_best} url={`brand/status/${cat._id}`} flag="4" label="is_best" />
                </td>
                <td className="p-4">
                  <StatusBtn status={cat.is_featured} url={`brand/status/${cat._id}`} flag="5" label="is_featured" />
                </td>
                ...
                <td className="p-4 text-center">
                  <div className="flex justify-center gap-4">
                    <Link
                      href={`/admin/brand/edit/${cat._id}`}
                      className="text-blue-600 hover:text-blue-800 font-medium text-sm"
                    >
                      Edit
                    </Link>

                    <Delete url={ `brand/delete/${cat._id}`} />
                  </div>
                </td>

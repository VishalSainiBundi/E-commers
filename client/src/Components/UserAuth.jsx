...
const API_IMAGE_URL = process.env.NEXT_PUBLIC_API_BASE_URL
  ? `${process.env.NEXT_PUBLIC_API_BASE_URL}img/product/main-image/`
  : "https://e-commers-tj4b.onrender.com/img/product/main-image/";
...
export default function UserAuth() {
  ...
  // Persisted form values so OTP screen can go back
  ...
  // ── Sync cart/wishlist after successful auth ───────────────────────────────
    ...
    if (cartResult.data.flag === 0) {
      const syncedCart = (cartResult.data.finalCart || [])
        ...
        .map((item) => ({
          ...
          qty: item.quantity,
          imgUrl: API_IMAGE_URL + item.product_id.thumbnail,
  ...
  // ── STEP 1: Submit credentials → call send-otp ────────────────────────────
  ...
  // ── STEP 2: Verify OTP → issue token → redirect ───────────────────────────
  const verifyOtpHandler = async () => {
    ...
    try {
      ...
      try {
        ...
        // cart sync failure is non-fatal
      }
      notify(isLogin ? "Logged in successfully!" : "Account created & logged in!", 0);
  ...
  const resendOtp = async () => {
    ...
    try {
      ...
      if (response.data.flag !== 0) {
        const secondsLeft = response.data.secondsLeft;
      ...
      notify(err?.response?.data?.msg || "Could not resend OTP. Try again.", 1);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
        <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-8">
          ...
          {/* Error */}
          {otpError && (
            <p className="text-center text-sm text-red-500 mt-2 mb-4">{otpError}</p>
          )}
          ...
          <button
            ...
            className="w-full mt-6 bg-teal-500 disabled:bg-teal-300 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl transition"
          >
      ...
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg flex overflow-hidden">
        ...
        <div className="w-full md:w-1/2 p-8">
          ...
          <p className="text-xs text-center text-gray-400 mt-4">
            {isLogin ? "A one-time code will be sent to your email." : "A verification code will be sent to confirm your email."}
          </p>
        </div>
      </div>
    </div>

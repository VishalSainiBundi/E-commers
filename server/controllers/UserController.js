...
const register = async (req, res) => {
  try {
    ...
    if (!name || !email || !password || password.length < 6) {
      return res.status(400).send({ msg: "Name, valid email and a password of at least 6 characters are required", flag: 1 });
    }

    const user = await userModel.findOne({ email });
    if (user) {
      return res.send({ msg: "Try another email id", flag: 1 });
    }
...
const token = createToken({...user.toJSON(),password:null})
    return res.send({
      ...
      flag: 0,
      user: { ...user.toJSON(), password: null},token
    });
  } catch (error) {
